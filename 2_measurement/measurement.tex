...
下記のFig.\ref{RC}に示したRC回路を用い、Vinに入力を与えるとVoutで信号が立ち上がるまでに
...
今回、足関節ロボットの骨格としてFig.\ref{bodyBone}人体模型を用いた。本模型から必要となる、膝より下の部分を取り外した。
取り外した結果、Fig.\ref{legBone}の様になった。
\begin{figure}[h]
    \begin{center}
...
固定を行った。そして、人間の筋付着位置に合わせて空気圧人工筋を接続するため、リングボルトを設置した。その結果、下記のFig.\ref{legJoint}の様になった。
...
重量、重心位置の調整を行った結果、Fig.\ref{legPb}の様になった。
